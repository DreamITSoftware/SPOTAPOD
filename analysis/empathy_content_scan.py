#!/usr/bin/env python3
"""
empathy_content_scan.py

Counts mentions of empathy-related terms (empathy, empathetic,
empathize) across post content in all three datasets. Same discipline
...
isn't just a checkbox for leaders" post already found in
career-transitions-content.md's burnout-scan check -- confirming it's a
genuinely recurring, cross-category leadership template rather than a
one-off found by coincidence in a single scan.

Usage:
    python3 empathy_content_scan.py podawaa /path/to/podawaa2024.json
    python3 empathy_content_scan.py hyperclapper /path/to/HyperClaper.json
    python3 empathy_content_scan.py linkboost /path/to/LinkBoost-2025.json
"""
import sys
import json
import argparse
...
TERMS = {
    ...
    "empathize": r"\bempathi[sz]e[sd]?\b",
}
...
def scan(texts, label, n):
    ...
    print(f"Records mentioning any empathy-related term: {any_hit:,} ({any_hit/n*100:.3f}%)")
