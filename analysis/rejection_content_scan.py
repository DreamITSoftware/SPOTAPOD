#!/usr/bin/env python3
"""
rejection_content_scan.py

Counts mentions of rejection-related terms (rejection(s), rejected,
reject, job rejection, rejection letter) across post content in all
...
ChatGPT" template (see decision-points.md) to confirm it isn't simply
re-surfacing that same template under a different keyword. It isn't --
zero overlap between the two sets, confirming these are genuinely
separate job-rejection posts.

Usage:
    python3 rejection_content_scan.py podawaa /path/to/podawaa2024.json
    python3 rejection_content_scan.py hyperclapper /path/to/HyperClaper.json
    python3 rejection_content_scan.py linkboost /path/to/LinkBoost-2025.json
"""
import sys
import json
import argparse
...
TERMS = {
    "rejection(s)": r"\brejections?\b",
    "rejected": r"\brejected\b",
    "reject": r"\brejects?\b",
    "job rejection": r"\bjob rejections?\b",
    "rejection letter": r"\brejection letters?\b",
}
...
def scan(texts, label, n):
    ...
    print(f"Records mentioning any rejection-related term: {any_hit:,} ({any_hit/n*100:.3f}%)")
