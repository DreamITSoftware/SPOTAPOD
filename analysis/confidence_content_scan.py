#!/usr/bin/env python3
"""
confidence_content_scan.py

Counts mentions of confidence-related terms (confidence, confident,
self-confidence, confidence building/booster) across post content in
...
niche-topic scans in this repo (1.1-5.2% "any mention"), well above
most other categories checked (law, nonprofit, cybersecurity, travel)
and in the same range as toplist-content.md.

Usage:
    python3 confidence_content_scan.py podawaa /path/to/podawaa2024.json
    python3 confidence_content_scan.py hyperclapper /path/to/HyperClaper.json
    python3 confidence_content_scan.py linkboost /path/to/LinkBoost-2025.json
"""
import sys
import json
import argparse
...
TERMS = {
    "confidence": r"\bconfidences?\b",
    "confident": r"\bconfident(ly)?\b",
    "self-confidence": r"\bself-confidence\b",
    "confidence building/booster": r"\bconfidence building\b|\bconfidence booster\b",
}
...
def scan(texts, label, n):
    ...
    print(f"Records mentioning any confidence-related term: {any_hit:,} ({any_hit/n*100:.3f}%)")
