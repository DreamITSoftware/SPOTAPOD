#!/usr/bin/env python3
"""
honesty_language_scan.py

Counts mentions of kindness, lying/lies/liar, and deception/deceptive
language across post content in all three datasets. Same discipline as
...
Related to authenticity-content.md, which covers self-description
("authentic," "honest truth," "real talk") rather than this scan's
focus on the underlying moral vocabulary of kindness, dishonesty, and
deception. Bare "kind" and bare "lie" are deliberately excluded from
their respective terms -- "kind" collides heavily with filler usage
("what kind of," "kind regards," "this kind of") and "lie" collides
with the unrelated physical sense ("lie down"), so both terms require
a qualifying form (kindness, be kind, lying, lies, liar, lied) to stay
reliable.

Usage:
    python3 honesty_language_scan.py podawaa /path/to/podawaa2024.json
    python3 honesty_language_scan.py hyperclapper /path/to/HyperClaper.json
    python3 honesty_language_scan.py linkboost /path/to/LinkBoost-2025.json
"""
import sys
import json
import argparse
...
TERMS = {
    "kindness": r"\bkindness\b|\bbe kind\b|\bacts? of kindness\b|\bkind gesture\b",
    "lying / lies / liar": r"\blying\b|\blies\b|\bliars?\b|\blied\b",
    "deception / deceptive / deceit": r"\bdeception\b|\bdeceptive\b|\bdeceit\b|\bdeceiving\b|\bdeceived\b",
}
...
def scan(texts, label, n):
    ...
    print(f"Records mentioning any kindness/lying/deception term: {any_hit:,} ({any_hit/n*100:.3f}%)")
