...
- [HyperClapper: external product context](docs/research/hyperclapper-product-context.md) — publicly sourced context on the product that produced `HyperClaper.json` (self-reported marketing claims vs. an independent, uncorroborated extension-usage tracker); draws no new statistic from the dataset itself
- [Federal agency mentions](docs/research/federal-agency-mentions.md) — named U.S. federal agency counts with a case-sensitivity fix for ICE/SEC collisions; computed from `hyperclapper-gamers.json` (see the provenance caveat in the doc), with independent term-count agreement against national-security-content.md and pharma-content.md's published `HyperClaper.json` figures
- [Full cross-check: hyperclapper-gamers.json](docs/research/hyperclapper-gamers-verification.md) — all 30 applicable analysis scripts run against `hyperclapper-gamers.json`, compared line-by-line against every published `HyperClaper.json` figure; zero discrepancies found
- [Enforcement precedent](docs/research/enforcement-precedent.md) — real FTC/AG/SEC/DOJ actions on fake social media engagement (Devumi, Sunday Riley, the § 465 warning-letter wave, the Constantinescu securities-fraud case); notes no public action was found against Podawaa, HyperClapper/LinkBoost, or Lempod specifically
