#!/usr/bin/env python3
"""
geographic_mentions_scan.py

Counts mentions of countries/regions as CONTENT SUBJECT MATTER (a post
discussing India, China, Europe, etc.) across post content in all three
datasets. This is distinct from demographics.md, which covers
self-reported location METADATA about accounts, not what a post's text
is actually about. Same discipline as every other scan in this repo:
aggregate counts only, no record, author, or matched text ever printed
beyond the duplicate-template check below.

Usage:
    python3 geographic_mentions_scan.py podawaa /path/to/podawaa2024.json
    python3 geographic_mentions_scan.py hyperclapper /path/to/HyperClaper.json
    python3 geographic_mentions_scan.py linkboost /path/to/LinkBoost-2025.json
"""
import sys
import json
import argparse
...
TERMS = {
    "India": r"\bIndia\b",
    "China": r"\bChina\b",
    "United States / America": r"\bUnited States\b|\bU\.S\.A?\.\b",
    "United Kingdom / Britain": r"\bUnited Kingdom\b|\bBritain\b",
...
def scan(texts, label, n):
    ...
    print(f"Records mentioning any tracked country/region: {any_hit:,} ({any_hit/n*100:.3f}%)")
